...
// ─── Explore.tsx ───────────────────────────────────────────────────────────
fs.writeFileSync(srcDir + '/pages/Explore.tsx', `import React, { useState, useMemo } from 'react'
...
const diffLabels: Record<number,string> = { 1:'Sehr leicht', 2:'Leicht', 3:'Moderat', 4:'Anspruchsvoll', 5:'Alpin/Extrem' }
...
export default function Explore() {
  ...
  const filtered = useMemo(() => allPins.filter(p => {
    ...
    if (searchQ.trim()) { const q = searchQ.toLowerCase(); return p.location.toLowerCase().includes(q) || p.story.toLowerCase().includes(q) }
    return true
      ...
      <div className="page-header">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            <button onClick={() => { triggerHaptic(15); setIsSubmitOpen(true) }} className="btn btn-primary text-xs py-2 px-3 font-bold shadow-lg">
        ...
        <div className="bg-[#152539] p-4 rounded-xl border border-[rgba(201,168,76,0.2)] space-y-3">
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="text" value={searchQ} onChange={e => setSearchQ(e.target.value)}
              placeholder="Suche nach Bucht, Wald, Taverne..." className="field flex-1" />
            <div className="flex gap-2 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap pt-2 border-t border-[rgba(201,168,76,0.1)]">
            ...
            {(['all',1,2,3,4,5] as const).map(d => (
              <button key={d} onClick={() => { triggerHaptic(10); setFilterDiff(d) }}
                  ...
                  <div>
                    <div className="relative h-44">
                      ...
                      <div className="absolute top-2 left-2 flex gap-1 flex-wrap max-w-[75%]">
                        {pin.dogFriendly && <span className="font-mono text-[0.58rem] bg-emerald-950/90 border border-emerald-500/60 text-emerald-300 px-2 py-0.5 rounded-full font-bold">🐕 Hund</span>}
                        {pin.strollerFriendly && <span className="font-mono text-[0.58rem] bg-blue-950/90 border border-blue-500/60 text-blue-300 px-2 py-0.5 rounded-full font-bold">👶 Kinderwagen</span>}
                    ...
                    <div className="p-4">
                      ...
                      {unlocked
                        ? <div className="bg-[#0C1825] rounded-lg px-3 py-2 flex items-center gap-2 mb-3"><span className="text-emerald-400 text-xs">📍</span><span className="coord text-emerald-400 font-bold">{pin.gps}</span></div>
                        : <button onClick={() => scratchSecret(pin.id, pin.xp, pin.location, pin.gps, pin.category)} className="btn btn-secondary w-full text-xs py-2 mb-3 font-bold">🪙 GPS freischalten</button>
                      }
                    </div>
                  </div>
                  <div className="px-4 pb-4 pt-1 border-t border-[rgba(201,168,76,0.1)] flex gap-2">
                    <button onClick={() => { triggerHaptic(10); window.open(createGoogleCalendarUrl({ title: pin.location, description: pin.story, location: pin.location }), '_blank') }}
                      className="btn btn-ghost flex-1 text-[0.62rem] py-1.5">📅 Kalender</button>
                    <button onClick={() => setStoryModal({ isOpen:true, title:pin.location, location:pin.tag, gps:pin.gps, xp:pin.xp, image:pin.image })}
                      className="btn btn-ghost flex-1 text-[0.62rem] py-1.5">📲 Story</button>
                  </div>
                </div>
              )
            })}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {activities.map(a => (
              <div key={a.id} className="card p-4 flex flex-col justify-between">
                ...
                <button onClick={() => setResModal({ isOpen:true, hostName:a.name, city:'Portugal', category:a.cat })}
                  className="btn btn-secondary w-full text-xs py-2">Anfragen (0% Fee) →</button>
              </div>
            ))}
          </div>
        </div>

        <StoryGeneratorModal isOpen={storyModal.isOpen} onClose={() => setStoryModal(p => ({ ...p, isOpen:false }))} data={storyModal} />
        <ReservationModal isOpen={resModal.isOpen} onClose={() => setResModal(p => ({ ...p, isOpen:false }))} hostName={resModal.hostName} city={resModal.city} category={resModal.category} />
        <SubmitSpotModal isOpen={isSubmitOpen} onClose={() => setIsSubmitOpen(false)} onSuccess={spot => {
          const newPin: StoryPin = {
            id: Date.now(), local: 'Du (Community)', avatar: 'YOU',
            location: spot.location + ' — ' + spot.title, story: spot.insiderStory,
            rating: 5.0, reviews: 1, gps: 'GPS ausstehend', locked: false,
            ...
            category: spot.category, xp: 150, difficulty: spot.difficulty,
            dogFriendly: spot.dogFriendly, dogDetails: spot.dogNotes,
            strollerFriendly: spot.strollerFriendly, strollerDetails: spot.strollerNotes,
