...
# Kontenlage WpHG / MiFID-II / MAR Guardrails v5

## 0. Status

Dieser Skill ist technische Governance, keine Rechtsberatung.

Er behauptet niemals:
- "rechtssicher"
- "lizenzfrei"
- "automatisch ausgenommen"

Die konkrete Produkt-/User-Journey muss juristisch geprüft werden, bevor ein Grenzbereich live geht.
...
Alle drei werden separat protokolliert.

## 2. Class A-F

### A — Fakt
Reine überprüfbare Information.

### B — Deskriptiver Kategorievergleich
Keine persönliche Handlungsaussage.

### C — Fester Archetyp / Bildungsprofil
Statische, redaktionelle Typen.

### D — Szenario / Stress Test / Prognose
Explizit hypothetisch, historisch oder modelliert.
...
Eine rein deskriptive Erwähnung eines konkreten Instruments ist nicht automatisch verboten.

Kritisch wird insbesondere:
```text
specific_instrument = true
AND
recommendatory_context = true
```
...
Bei Unsicherheit:
→ höhere Risikoklasse verwenden
→ nicht veröffentlichen.

## 5. WpHG/MiFID-II-Prüfpfad

Der Prüfpfad berücksichtigt mindestens:
- Recommendation vorhanden?
- Bezug auf Finanzinstrument/Transaktion?
- persönliche Empfehlung?
- als geeignet dargestellt oder auf persönliche Umstände gestützt?
- öffentliche oder nicht ausschließlich öffentliche Verbreitung?
...
Wenn der Output eine Investment Recommendation oder Information mit möglicher Strategie-/Wertlenkung zu Finanzinstrumenten darstellt:
→ MAR Review.
...
Keine Provisionen/Affiliate-Links sind Governance-Vorgaben, ersetzen aber nicht diese Prüfung.
...
Das LLM darf nicht:
- E/F zu B/C heruntersetzen
- einen Block aufheben
- selbst eine Ausnahme erzeugen.

## 8. Disclosure Contract

Jede sensible Finanzpublikation muss die erforderlichen Metadaten tragen:
...
- komplette Typenübersicht sichtbar
- keine Rückspiegelung sensibler Nutzerdaten
...
Nicht:
> "X ist für dich geeignet."

## 10. DeFi

Standardmäßig konservative Einstufung.
...
```text
if data_status == RED:
    BLOCK_NEW_PUBLICATION

if final_class in [E,F]:
    BLOCK

if missing_provenance:
    BLOCK

if required_disclosure_missing:
    BLOCK

if legal_fact_without_jurisdiction_or_scope:
    BLOCK

if stale_data_exceeds_hard_ttl:
    BLOCK

if compliance_engine_error:
    FAIL_CLOSED
```
...
Bei:
- Parserfehler
- Policy Engine nicht erreichbar
- unklarer Klassifikation
- fehlender Datenherkunft
- widersprüchlicher Regelversion
